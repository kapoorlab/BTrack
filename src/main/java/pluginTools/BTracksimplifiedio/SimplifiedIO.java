...
public class SimplifiedIO {
	...
	/**
	 * Loads an image using ImageJ1, then wraps it into an ImgPlus object Returns
	 * null if the image is not in a supported format. Quick and dirty it either
	 * works or does not
	 *
	...
	/**
	 * Saves the specified image to the specified file path. The specified image is
	 * saved as a "tif" if there is no extension.
	 * <p>
	...
	private static <T extends NativeType<T>> ImgPlus<T> convertBetweenRealType(ImgPlus image, RealType type) {
		...
		Img<T> convertedImg = ImgView.wrap(convertedRAI,
				new ArrayImgFactory<T>(convertedRAI.randomAccess().get().createVariable()));
