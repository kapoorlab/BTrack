...
//	private final HashMap< ImagePlus, FloatingDisplayConfigFrame > configFrames = new HashMap<>();
	...
	public void mouseClicked( final MouseEvent e )
	{
		...
		case 1:
		{
			...
			if ( null != editedBCellobject ) { return; }
...
//		}
		...
		case KeyEvent.VK_SPACE:
		{
			...
			{
				final int frame = displayer.imp.getFrame() - 1;
				if ( null == quickEditedBCellobject ) { return; }
			}
			e.consume();
			break;
	...
	private void updateStatusBar( final BCellobject BCellobject, final String units )
	{
		...
		{
			statusString = String.format( Locale.US, "BCellobject ID%d, x = %.1f, y = %.1f, z = %.1f, r = %.1f %s", BCellobject.ID(), BCellobject.getFeature( BCellobject.POSITION_X ), BCellobject.getFeature( BCellobject.POSITION_Y ), BCellobject.getFeature( BCellobject.RADIUS ), units );
		}
		else
		{
			statusString = String.format( Locale.US, "BCellobject %s, x = %.1f, y = %.1f, z = %.1f, r = %.1f %s", BCellobject.getName(), BCellobject.getFeature( BCellobject.POSITION_X ), BCellobject.getFeature( BCellobject.POSITION_Y ), BCellobject.getFeature( BCellobject.RADIUS ), units );
		}
