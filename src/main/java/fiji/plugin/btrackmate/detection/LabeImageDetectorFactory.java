...
import static fiji.plugin.btrackmate.detection.DetectorKeys.DEFAULT_TARGET_CHANNEL;
import static fiji.plugin.btrackmate.detection.DetectorKeys.KEY_TARGET_CHANNEL;
import static fiji.plugin.btrackmate.detection.ThresholdDetectorFactory.KEY_SIMPLIFY_CONTOURS;
...
import static fiji.plugin.btrackmate.io.IOUtils.writeTargetChannel;
...
public class LabeImageDetectorFactory<T extends RealType<T> & NativeType<T>> implements SpotDetectorFactory<T> {
	...
	public static final String INFO_TEXT = "<html>"
			+ "This detector creates spots by by importing regions from a label image." + "<p>"
			...
			+ "that is unique to the object." + "<p>"
			...
			+ "of the same volume that the label are created." + "</html>";
	...
	public SpotDetector<T> getDetector(final Interval interval, final int frame) {
		...
		final LabelImageDetector<T> detector = new LabelImageDetector<>(imFrame, interval, calibration,
				simplifyContours);
		return detector;
	}
