...
public class BUDDYFeatureModel {
	...
	/**
	 * Instantiates a new feature model. The basic Budpointobject features
	 * (POSITON_*, RADIUS, FRAME, QUALITY) are declared. Edge and track feature
	 * declarations are left blank.
	 *
	...
	protected BUDDYFeatureModel(final BUDDYModel model) {
		...
		declareBudpointobjectFeatures(Budpointobject.FEATURES, Budpointobject.FEATURE_NAMES,
				Budpointobject.FEATURE_SHORT_NAMES, Budpointobject.FEATURE_BUDDYDIMENSIONS, Budpointobject.IS_INT);
	}
	 ...
	 * feature.
	 *
	 * @param trackFeature the track feature to parse. Throw an
	 *                     {@link IllegalArgumentException} if the feature is
	 *                     unknown.
	 ...
	 * feature.
	 *
	 * @param edgeFeature the track feature to parse. Throw an
	 *                    {@link IllegalArgumentException} if the feature is
	 *                    unknown.
	 * @param visibleOnly if <code>true</code>, will only include edges in visible
	 ...
	 * @param featureBUDDYDimensions the BUDDYDimension of these features.
	...
	public void declareEdgeFeatures(final Collection<String> features, final Map<String, String> featureNames,
			final Map<String, String> featureShortNames, final Map<String, BUDDYDimension> featureBUDDYDimensions,
		...
		for (final String feature : features) {
			...
			if (null == BUDDYDimension) {
				throw new IllegalArgumentException("Feature " + feature + " misses a BUDDYDimension.");
			}
	...
	/**
	 * Returns the BUDDYDimension mapping of the edge features that are dealt with
	 * in this model.
	 *
	...
	/*
	 * Budpointobject FEATURES the Budpointobject features are stored in the
	 * Budpointobject object themselves, but we declare them here.
	 */
	 ...
	 * the map misses a feature.
	 *
	 * @param features               the list of Budpointobject feature to register.
	 * @param featureNames           the name of these features.
	 * @param featureShortNames      the short name of these features.
	 * @param featureBUDDYDimensions the BUDDYDimension of these features.
	 * @param isIntFeature           whether some of these features are made of
	 ...
	 *                               <code>double</code>s (<code>false</code> ).
	 */
	public void declareBudpointobjectFeatures(final Collection<String> features, final Map<String, String> featureNames,
			final Map<String, String> featureShortNames, final Map<String, BUDDYDimension> featureBUDDYDimensions,
			final Map<String, Boolean> isIntFeature) {
	...
	/**
	 * Returns the name mapping of the Budpointobject features that are dealt with
	 * in this model.
	 *
	...
	/**
	 * Returns the short name mapping of the Budpointobject features that are dealt
	 * with in this model.
	 *
	...
	/**
	 * Returns the BUDDYDimension mapping of the Budpointobject features that are
	...
	public String toString() {
		...
		appendFeatureDeclarations(str, BudpointobjectFeatures, BudpointobjectFeatureNames,
				BudpointobjectFeatureShortNames, BudpointobjectFeatureBUDDYDimensions, BudpointobjectFeatureIsInt);
		str.append('\n');

		// Edges
		str.append("Edge features declared:\n");
		appendFeatureDeclarations(str, edgeFeatures, edgeFeatureNames, edgeFeatureShortNames,
				edgeFeatureBUDDYDimensions, edgeFeatureIsInt);
		str.append('\n');

		// Track
		str.append("Track features declared:\n");
		appendFeatureDeclarations(str, trackFeatures, trackFeatureNames, trackFeatureShortNames,
				trackFeatureBUDDYDimensions, trackFeatureIsInt);
		str.append('\n');
	...
	private static final void appendFeatureDeclarations(final StringBuilder str, final Collection<String> features,
			...
			final Map<String, BUDDYDimension> featureBUDDYDimensions, final Map<String, Boolean> isIntFeature) {
		for (final String feature : features) {
			str.append("   - " + feature + ": " + featureNames.get(feature) + ", '" + featureShortNames.get(feature)
					+ "' (" + featureBUDDYDimensions.get(feature) + ")");
