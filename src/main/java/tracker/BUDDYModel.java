...
public class BUDDYModel {
	 ...
	 * the events listed in {@link BudModelChangeEvent}, namely
	 * <ul>
	 ...
	 * </ul>
	 * The {@link BudModelChangeEvent#MODEL_MODIFIED} cannot be cached this way, for
	 * it needs to be configured with modification Budpointobject and edge targets,
	 * so it uses a different system (see {@link #flushUpdate()}).
	 */
	 ...
	 *                 {@link BudModelChangeEvent#TRACKS_COMPUTED} event.
	 */
	public void setTracks(final SimpleWeightedGraph<Budpointobject, DefaultWeightedEdge> graph,
	...
	public void clearBudpointobjects(final boolean doNotify) {
		...
		if (doNotify) {
			final BudModelChangeEvent event = new BudModelChangeEvent(this,
					BudModelChangeEvent.Budpointobject_COMPUTED);
			for (final BudModelChangeListener listener : BudModelChangeListeners)
				listener.modelChanged(event);
		}
	}
	 ...
	 *                        {@link BudModelChangeEvent#BudpointobjectS_COMPUTED}
	 *                        event.
	 * @param Budpointobjects the {@link BudpointobjectCollection} to set.
	 */
	...
	/**
	 * Moves a single Budpointobject from a frame to another, make it visible if it
	 * was not, then mark it for feature update. If the source Budpointobject could
	 * not be found in the source frame, nothing is done and <code>null</code> is
	 * returned.
	 * <p>
	 ...
	 * </pre>
	 *
	 * @param BudpointobjectToMove the Budpointobject to move
	 * @param fromFrame            the frame the Budpointobject originated from
	 * @param toFrame              the destination frame
	 * @return the Budpointobject that was moved, or <code>null</code> if it could
	 *         not be found in the source frame
	 */
	public synchronized Budpointobject moveBudpointobjectFrom(final Budpointobject BudpointobjectToMove,
			final Integer fromFrame, final Integer toFrame) {
		final boolean ok = Budpointobjects.remove(BudpointobjectToMove, fromFrame);
		if (!ok) {
			if (DEBUG) {
				System.err.println("[TrackMateModel] Could not find Budpointobject " + BudpointobjectToMove
						+ " in frame " + fromFrame);
			}
			return null;
		}
		...
		if (DEBUG) {
			System.out.println("[TrackMateModel] Moving " + BudpointobjectToMove + " from frame " + fromFrame
					+ " to frame " + toFrame);
		}
	...
	/**
	 * Adds a single Budpointobject to the collections managed by this model, mark
	 * it as visible, then update its features.
	 * <p>
	 * For the model update to happen correctly and listeners to be notified
	 * properly, a call to this method must happen within a transaction, as in:
	 *
	 ...
	 * </pre>
	 * 
	 * @param BudpointobjectToAdd the Budpointobject to add.
	 * @param toFrame             the frame to add it to.
	 *
	 * @return the Budpointobject just added.
	 */
	public synchronized Budpointobject addBudpointobjectTo(final Budpointobject BudpointobjectToAdd,
		...
		if (DEBUG) {
			System.out
					.println("[TrackMateModel] Adding Budpointobject " + BudpointobjectToAdd + " to frame " + toFrame);
		}
	...
	/**
	 * Removes a single Budpointobject from the collections managed by this model.
	 * If the Budpointobject cannot be found, nothing is done and <code>null</code>
	 * is returned.
	 * <p>
	 * For the model update to happen correctly and listeners to be notified
	 * properly, a call to this method must happen within a transaction, as in:
	 ...
	 * </pre>
	 *
	 * @param BudpointobjectToRemove the Budpointobject to remove.
	 * @return the Budpointobject removed, or <code>null</code> if it could not be
	 *         found.
	 */
	public synchronized Budpointobject removeBudpointobject(final Budpointobject BudpointobjectToRemove) {
		...
		if (Budpointobjects.remove(BudpointobjectToRemove, fromFrame)) {
			...
			if (DEBUG)
				System.out.println("[TrackMateModel] Removing Budpointobject " + BudpointobjectToRemove + " from frame "
						+ fromFrame);
		...
		if (DEBUG)
			System.err.println("[TrackMateModel] The Budpointobject " + BudpointobjectToRemove
	...
	/**
	 * Mark the specified Budpointobject for update. At the end of the model
	 ...
	 * </pre>
	 *
	 * @param BudpointobjectToUpdate the Budpointobject to mark for update
	 */
	 ...
	 * </pre>
	 *
	 * @param source the source Budpointobject.
	 * @param target the target Budpointobject.
	 * @param weight the weight of the edge.
	 * @return the edge created.
	 */
	public synchronized DefaultWeightedEdge addEdge(final Budpointobject source, final Budpointobject target,
			final double weight) {
	...
	private void flushUpdate() {
		 ...
		 * Budpointobjects have been removed that causes edges to be removes, we already
		 * know about it). We do NOT recompute tracks if Budpointobjects have been
		...
		final int nBudpointobjectsToUpdate = BudpointobjectsAdded.size() + BudpointobjectsMoved.size()
				+ BudpointobjectsUpdated.size();
