...
public class NNsearch implements BudTracker {
	...
	public NNsearch(
			final ArrayList<ArrayList<Budobject>> Allblobs, 
			final long maxframe){
	...
	public boolean process() {
		...
		for (int frame = 0; frame < maxframe - 1; ++frame){
			...
			for (int index = 0; index < Spotmaxtarget.size(); ++index){
				...
				System.out.println(Spotmaxtarget.get(index).Budcenter.getDoublePosition(0) + " " + Spotmaxtarget.get(index).Budcenter.getDoublePosition(1)  );
			...
			while(baseobjectiterator.hasNext()){
				...
				System.out.println("Source" + source.Budcenter.getDoublePosition(0) + " " + source.Budcenter.getDoublePosition(1)  );
				...
				synchronized (graph) {
					...
					System.out.println("Source" + source.Budcenter.getDoublePosition(0) + " " + source.Budcenter.getDoublePosition(1)  + " "+ source.hashCode());
					System.out.println("targetNode" + targetNode.getValue().getDoublePosition(0) + " " + targetNode.getValue().getDoublePosition(1) + " " + targetNode.hashCode()  );
