 ...
 * <p>
 * A convex branch is a portion of a track for which all BCellobjects - but the
 * first and last one - have exactly one predecessor and one successor (in
 * time). The first and last BCellobjects of a branch may have 0 or 1 or more
 ...
 * have branches that contain a BCellobject with more than one predecessor and
 ...
 * <p>
 * Some branches may have gaps in them, that is two BCellobjects separated by
 * more than one frame. By default this does not lead to cutting the branch in
 * two. If you want to force branches to contain BCellobjects that are separated
 ...
 * </pre>
 * <p>
 * It is ensured that each BCellobject in the model is present in exactly one
 * branch of the decomposition. Only BCellobjects belonging to visible tracks
...
public class ConvexBranchesDecomposition implements Algorithm, Benchmark {
	 ...
	 *            <code>true</code>, links will only reach for these BCellobjects.
	 *            If <code>false</code>, a link can target a BCellobject within a
	...
	public boolean checkInput() {
		...
		for (final DefaultWeightedEdge edge : tm.edgeSet()) {
			...
			if (source.diffTo(target, BCellobject.POSITION_T) == 0d) {
				errorMessage = BASE_ERROR_MSG + "Cannot deal with links between two BCellobjects in the same frame ("
						+ source + " & " + target + ").\n";
				return false;
			}
		}
	 ...
	 *            some BCellobjects inside a branch may be a fusion or splitting
	 *            point. This leads to fewer and longer branches.
	 * @param forbidGaps
	 *            if <code>true</code>, two neighbor BCellobjects in a branch will
				...
				if (BCellobject.diffTo(next, BCellobject.POSITION_T) < 2) {
					...
					for (final BCellobject predecessor : predecessors) {
						if (!forbidMiddleLinks && !found
								&& BCellobject.diffTo(predecessor, BCellobject.POSITION_T) < 2) {
							found = true;
						} else {
							graph.removeEdge(predecessor, BCellobject);
							links.add(makeLink(predecessor, BCellobject));
	 ...
	 */
	public static final SimpleDirectedGraph<List<BCellobject>, DefaultEdge> buildBranchGraph(
			final TrackBranchDecomposition branchDecomposition) {
		final SimpleDirectedGraph<List<BCellobject>, DefaultEdge> branchGraph = new SimpleDirectedGraph<>(
				DefaultEdge.class);
		...
		for (final List<BCellobject> link : links) {
			...
			if (targetBranch == null) {
				/*
				 * We could not find this link's target in the map of first BCellobjects. Most
				 * likely this means that the link targets a middle BCellobject, because the
	 ...
	 * <p>
	 * Branches are returned as list of BCellobject. It is ensured that the
	 * BCellobjects are ordered in the list by increasing frame number, and that two
	 ...
	 * is ensured that the first element of all links is the last BCellobject of a
	 * branch, and the second element of this link is the first BCellobject of
	 * another branch. Otherwise, a link can target a BCellobject within a branch.
	 *
