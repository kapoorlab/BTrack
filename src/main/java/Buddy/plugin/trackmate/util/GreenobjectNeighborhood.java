...
public class GreenobjectNeighborhood<T extends RealType<T>> implements Neighborhood<T> {
	...
	public GreenobjectNeighborhood(final Greenobject Greenobject, final ImgPlus<T> img) {
		this.calibration = TMUtils.getSpatialCalibration(img);
		// Center
		this.center = new long[img.numDimensions()];
		for (int d = 0; d < center.length; d++) {
			center[d] = Math
					.round(Greenobject.getFeature(Greenobject.POSITION_FEATURES[d]).doubleValue() / calibration[d]);
		}
		// Span
		final long[] span = new long[img.numDimensions()];
		for (int d = 0; d < span.length; d++) {
			span[d] = Math.round(Greenobject.getFeature(Greenobject.RADIUS) / calibration[d]);
		}
	...
	@Override
	public final GreenobjectNeighborhoodCursor<T> cursor() {
		return new GreenobjectNeighborhoodCursor<>(this);
	}

	@Override
	public GreenobjectNeighborhoodCursor<T> localizingCursor() {
		return cursor();
	}
	...
	@Override
	public GreenobjectNeighborhoodCursor<T> iterator() {
		return cursor();
	}
