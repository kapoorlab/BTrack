...
import Buddy.plugin.trackmate.tracking.kdtree.GreenNearestNeighborTracker;
 ...
 * 
 */
public class GreenModelTools {
	private GreenModelTools() {
	}

	/**
	 * Sets the content of the specified selection model to be the whole tracks the
	 * selected BCellobjects belong to. Other selected edges are removed from the
	 * selection.
	 * 
	 * @param selectionModel
	 *            the {@link SelectionModel} that will be updated by this call.
	 */
	public static void selectTrack(final GreenSelectionModel selectionModel) {
		selectionModel.clearEdgeSelection();
		selectionModel.selectTrack(selectionModel.getGreenobjectSelection(),
				Collections.<DefaultWeightedEdge>emptyList(), 0);
	}

	/**
	 * Sets the content of the specified selection model to be the whole tracks the
	 * selected BCellobjects belong to, but searched for only forward in time
	 * (downward). Other selected edges are removed from the selection.
	 * 
	 * @param selectionModel
	 *            the {@link SelectionModel} that will be updated by this call.
	 */
	public static void selectTrackDownward(final GreenSelectionModel selectionModel) {
		selectionModel.clearEdgeSelection();
		selectionModel.selectTrack(selectionModel.getGreenobjectSelection(),
				Collections.<DefaultWeightedEdge>emptyList(), -1);
	}

	/**
	 * Sets the content of the specified selection model to be the whole tracks the
	 * selected BCellobjects belong to, but searched for only backward in time
	 * (backward). Other selected edges are removed from the selection.
	 * 
	 * @param selectionModel
	 *            the {@link SelectionModel} that will be updated by this call.
	 */
	public static void selectTrackUpward(final GreenSelectionModel selectionModel) {
		selectionModel.clearEdgeSelection();
		selectionModel.selectTrack(selectionModel.getGreenobjectSelection(),
				Collections.<DefaultWeightedEdge>emptyList(), 1);
	}
	 ...
	 */
	public static void linkGreenobjects(final InteractiveGreen parent, final GreenModel model,
			final GreenSelectionModel selectionModel) {
		...
		final GreenNearestNeighborTracker tracker = new GreenNearestNeighborTracker(parent, settings);
		...
		if (!tracker.checkInput() || !tracker.process()) {
			System.err.println("Problem while computing Greenobject links: " + tracker.getErrorMessage());
			return;
		}
		final SimpleWeightedGraph<Greenobject, DefaultWeightedEdge> graph = tracker.getResult();
	 ...
	 *            to ensure that all edges have the target feature declared in the
	 *            specified {@link FeatureModel}.
	 * @param fm
	 ...
	 * @param feature
	 *            the feature to use for comparison. It is the caller responsibility
	 *            to ensure that all tracks have the target feature declared in the
	 *            specified {@link FeatureModel}.
	 * @param fm
	 *            the {@link FeatureModel} to read feature values from.
	 * @return a new {@link Comparator}.
	 */
	public final static Comparator<Integer> featureTrackComparator(final String feature, final GreenFeatureModel fm) {
