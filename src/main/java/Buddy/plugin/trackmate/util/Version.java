...
public class Version implements Comparable<Version> {
	...
	public int compareTo(final Version that) {
		...
		for (int i = 0; i < length; i++) {
			final int thisPart = i < thisParts.length ? NumberParser.parseInteger(thisParts[i]) : 0;
			final int thatPart = i < thatParts.length ? NumberParser.parseInteger(thatParts[i]) : 0;
