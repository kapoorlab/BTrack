...
public class TmXmlReader_v12 extends TmXmlReader {
	...
	/**
	 * We must initialize the model with feature declarations that match the feature
	 * we retrieved from the file.
	 */
	private void declareDefaultFeatures(final FeatureModel fm) {
		// BCellobjects:
		fm.declareBCellobjectFeatures(BCellobject.FEATURES, BCellobject.FEATURE_NAMES, BCellobject.FEATURE_SHORT_NAMES,
				BCellobject.FEATURE_DIMENSIONS, BCellobject.IS_INT);
		;
		fm.declareBCellobjectFeatures(F_FEATURES, F_FEATURE_NAMES, F_FEATURE_SHORT_NAMES, F_FEATURE_DIMENSIONS,
				F_ISINT);
		...
		fm.declareTrackFeatures(TrackDurationAnalyzer.FEATURES, TrackDurationAnalyzer.FEATURE_NAMES,
				...
				TrackDurationAnalyzer.IS_INT);
	}

	/**
	 * Load the tracks, the track features and the ID of the visible tracks into the
	 * model modified by this reader.
	 *
	 * @param model
	 *            the model to feed.
	 */
	 ...
	 *         Return <code>null</code> if the track feature filters data cannot be
	 *         found in the file.
	 */
	 ...
	 * file. Return <code>null</code> if the BCellobject feature filters data cannot
	...
	/**
	 * Update the given {@link Settings} object with the TrackerSettings and
	 * {@link BCellobjectTracker} fields named {@link Settings#trackerSettings} and
	 * Settings#tracker read within the XML file this reader is initialized with.
	 * <p>
	 ...
	 * tracker info can be read, but cannot be understood (most likely because the
	 * class the XML refers to is unknown) then a default object is substituted.
	 *
	 * @param settings
	...
	private void getTrackerSettings(final Settings settings, final TrackerProvider provider) {
		...
		if (null == trackerClassName) {
			...
			if (trackerClassName.equals("Buddy.plugin.trackmate.tracking.SimpleFastLAPTracker")
					|| trackerClassName.equals("Buddy.plugin.trackmate.tracking.SimpleLAPTracker")) {
				// convert to simple fast version
				trackerKey = SimpleFastLAPTrackerFactory.THIS2_TRACKER_KEY;
			} else if (trackerClassName.equals("Buddy.plugin.trackmate.tracking.FastLAPTracker")
					|| trackerClassName.equals("Buddy.plugin.trackmate.tracking.LAPTracker")) {
		...
		// Deal with tracker settings
		{
			...
			final String trackerSettingsClassName = element
					.getAttributeValue(TRACKER_SETTINGS_CLASS_ATTRIBUTE_NAME_v12);
			...
			if (null == trackerSettingsClassName) {
				...
				if (trackerSettingsClassName.equals("Buddy.plugin.trackmate.tracking.LAPTrackerSettings")) {
					...
					if (trackerKey.equals(SimpleFastLAPTrackerFactory.THIS2_TRACKER_KEY)
							|| trackerKey.equals(FastLAPTrackerFactory.THIS_TRACKER_KEY)) {

						/*
						 * Read
						 */

						final double alternativeObjectLinkingCostFactor = readDoubleAttribute(element,
								TRACKER_SETTINGS_ALTERNATE_COST_FACTOR_ATTNAME_v12, Logger.VOID_LOGGER);
						final double cutoffPercentile = readDoubleAttribute(element,
								TRACKER_SETTINGS_CUTOFF_PERCENTILE_ATTNAME_v12, Logger.VOID_LOGGER);
						final double blockingValue = readDoubleAttribute(element,
								TRACKER_SETTINGS_BLOCKING_VALUE_ATTNAME_v12, Logger.VOID_LOGGER);
						// Linking
						...
						final boolean allowGapClosing = readBooleanAttribute(gapClosingElement,
								TRACKER_SETTINGS_ALLOW_EVENT_ATTNAME_v12, Logger.VOID_LOGGER);
						...
						final boolean allowSplitting = readBooleanAttribute(splittingElement,
								TRACKER_SETTINGS_ALLOW_EVENT_ATTNAME_v12, Logger.VOID_LOGGER);
						...
						final boolean allowMerging = readBooleanAttribute(mergingElement,
								TRACKER_SETTINGS_ALLOW_EVENT_ATTNAME_v12, Logger.VOID_LOGGER);
						...
						logger.error("\nTracker settings class (" + trackerSettingsClassName
								+ ") does not match tracker requirements (" + ts.getClass().getName() + "),\n");
						logger.error("substituting default values.\n");
					}

				} else if (trackerSettingsClassName
						.equals("Buddy.plugin.trackmate.tracking.kdtree.NearestNeighborTrackerSettings")) {
					...
					if (trackerKey.equals(NearestNeighborTrackerFactory.TRACKER_KEY)) {
						...
						final double maxDist = readDoubleAttribute(element, MAX_LINKING_DISTANCE_ATTRIBUTE,
								Logger.VOID_LOGGER);
	 ...
	 *
	 * @return a {@link BCellobjectCollection}. Return <code>null</code> if the
	 *         BCellobject section is not present in the file.
	 */
	private BCellobjectCollection getAllBCellobjects() {
		...
		if (null == BCellobjectCollection) {
			return null;
		}

		// Retrieve children elements for each frame
		final List<Element> frameContent = BCellobjectCollection
				.getChildren(BCellobject_FRAME_COLLECTION_ELEMENT_KEY_v12);
	...
	/**
	 * Return the filtered BCellobjects stored in this file, taken from the list of
	 * all BCellobjects, given in argument.
	 * <p>
	 * The {@link BCellobject} objects in this list will be the same that of the
	 * main list given in argument. If a BCellobject ID referenced in the file is in
	 * the selection but not in the list given in argument, it is simply ignored,
	 ...
	 *
	 * @return a {@link BCellobjectCollection}. Each BCellobject of this collection
	 *         belongs also to the given collection. Return <code>null</code> if the
	...
	private Map<Integer, Set<Integer>> getFilteredBCellobjectsIDs() {
		...
		if (null == selectedBCellobjectCollection) {
			return null;
		}

		final List<Element> frameContent = selectedBCellobjectCollection
				.getChildren(FILTERED_BCellobject_COLLECTION_ELEMENT_KEY_v12);
	...
	private boolean readDouble(final Element element, final String attName, final Map<String, Object> settings,
			final String mapKey) {
	...
	private boolean readInteger(final Element element, final String attName, final Map<String, Object> settings,
			final String mapKey) {
		final String str = element.getAttributeValue(attName);
		if (null == str) {
			errorMessage = "Attribute " + attName + " could not be found in XML element.";
	...
	private boolean readBoolean(final Element element, final String attName, final Map<String, Object> settings,
			final String mapKey) {
		final String str = element.getAttributeValue(attName);
		if (null == str) {
			errorMessage = "Attribute " + attName + " could not be found in XML element.";
	 ...
	 * TRACKER_SETTINGS_FEATURE_ELEMENT, fetch the feature attributes from them, and
