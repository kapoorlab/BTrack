...
public class SplittingCostFunction extends MultiThreadedBenchmarkAlgorithm implements OutputAlgorithm<Matrix> {
	...
	public SplittingCostFunction(final Map<String, Object> settings, final List<SortedSet<BCellobject>> trackSegments,
			final List<BCellobject> middlePoints) {
		...
		if (!allowSplitting) {
			...
			for (int ithread = 0; ithread < threads.length; ithread++) {
				...
				threads[ithread] = new Thread(
						"LAPTracker splitting cost thread " + (1 + ithread) + "/" + threads.length) {
					...
					public void run() {
						...
						for (int i = ai.getAndIncrement(); i < middlePoints.size(); i = ai.getAndIncrement()) {
							...
							for (int j = 0; j < trackSegments.size(); j++) {
								...
								final double cost = LAPUtils.computeLinkingCostFor(lStart, middle, maxDist,
										blockingValue, featurePenalties);
								m.set(i, j, cost);
							}
						}
					}
