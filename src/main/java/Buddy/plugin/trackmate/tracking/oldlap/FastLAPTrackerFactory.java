...
public class FastLAPTrackerFactory extends LAPTrackerFactory {
	...
	public static final String THIS_INFO_TEXT = "<html>"
			...
			+ "Tracking happens in 2 steps: First BCellobjects are linked from frame to frame to <br>"
			...
			+ "target BCellobjects, which makes this tracker suitable for Brownian motion.  <br> "
			+ "Penalties can be set to favor linking between BCellobjects that have similar  <br> " + "features. "
			+ "<p>" + "Solving the LAP relies on the Jonker-Volgenant solver, <br> "
			+ "that solves an assignment problem in O(n^3) instead of O(n^4)." + "<p>"
			+ "This is the old version of the LAP tracker (pre v2.5.0) that uses a dense cost matrix, and is therefore NOT suited for large problems."
			+ " </html>";
