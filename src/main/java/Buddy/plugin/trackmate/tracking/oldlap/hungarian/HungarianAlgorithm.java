...
public class HungarianAlgorithm implements AssignmentAlgorithm {
	...
	public int[][] computeAssignments(final double[][] matrix) {
		...
		// check if we have less than 1 line, 1 column, which causes normal algo to
		// hang.
		...
		// non negative values are the index of the starred or primed zero in the row or
		// column
		...
		while (!allAreCovered(coveredCols)) {
			...
			while (primedZero == null) {
				// keep making more zeroes until we find something that we can prime (i.e. a
				// zero that is uncovered)
	...
	/**
	 * The first step of the hungarian algorithm is to find the smallest element in
	 * each row and subtract it's values from all elements in that row
	 */
	 ...
	 * Init starred zeroes.
	 *
	 * for each column find the first zero if there is no other starred zero in that
	 * row then star the zero, cover the column and row and go onto the next column
	 *
	 * @param costMatrix
	 */
	 ...
	 * @param coveredCols
	 */
	private int[] primeSomeUncoveredZero(final double matrix[][], final int[] primesByRow, final int[] coveredRows,
			final int[] coveredCols) {
	 ...
	 * @param primesByRow
	 */
	private void incrementSetOfStarredZeroes(final int[] unpairedZeroPrime, final int[] starsByRow,
			final int[] starsByCol, final int[] primesByRow) {
