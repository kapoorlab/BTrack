...
import static Buddy.plugin.trackmate.tracking.TrackerKeys.DEFAULT_ALLOW_GAP_CLOSING;
import static Buddy.plugin.trackmate.tracking.TrackerKeys.DEFAULT_ALLOW_TRACK_MERGING;
import static Buddy.plugin.trackmate.tracking.TrackerKeys.DEFAULT_ALLOW_TRACK_SPLITTING;
...
import static Buddy.plugin.trackmate.tracking.TrackerKeys.DEFAULT_GAP_CLOSING_MAX_DISTANCE;
...
public class LAPUtils {
	...
	/**
	 * Compute the cost to link two BCellobjects, in the default way for the
	 * TrackMate trackmate.
	 * <p>
	 * This cost is calculated as follow:
	 * <ul>
	 * <li>The distance between the two BCellobjects <code>D</code> is calculated
	 * <li>If the BCellobjects are separated by more than the distance cutoff, the
	 * cost is set to be the blocking value. If not,
	 ...
	 *         check that all the BCellobject features used in penalties are indeed
	 *         found in all BCellobjects, because if such a feature is absent from
	 *         one BCellobject, the LAP trackers simply ignores the penalty and does
	 ...
	 *            tracking
	 * @param nBCellobjects
	 *            the number of middle BCellobjects to consider
	 * @param blockingValue
	 *            the blocking value for cost
	 * @param solutions
	 *            the Hungarian assignment couple
	 */
	public static final void displayCostMatrix(final double[][] costs, final int nSegments, final int nBCellobjects,
