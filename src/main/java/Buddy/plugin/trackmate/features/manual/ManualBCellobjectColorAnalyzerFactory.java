...
public class ManualBCellobjectColorAnalyzerFactory<T extends RealType<T> & NativeType<T>>
		implements BCellobjectAnalyzerFactory<T> {
	...
	@Override
	public BCellobjectAnalyzer<T> getAnalyzer(final Model model, final ImgPlus<T> img, final int frame,
			final int channel) {
