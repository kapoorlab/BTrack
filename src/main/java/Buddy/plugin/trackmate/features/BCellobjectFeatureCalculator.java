...
import Buddy.plugin.trackmate.features.spot.BCellobjectAnalyzer;
import Buddy.plugin.trackmate.features.spot.BCellobjectAnalyzerFactory;
import Buddy.plugin.trackmate.features.spot.IndependentBCellobjectFeatureAnalyzer;
...
public class BCellobjectFeatureCalculator extends MultiThreadedBenchmarkAlgorithm {
	...
	/**
	 * Calculates the BCellobject features configured in the {@link Settings} for
	 * all the BCellobjects of this model,
	 * <p>
	 * Features are calculated for each BCellobject, using their location, and the
	 * raw image. Since a {@link BCellobjectAnalyzer} can compute more than a
	 * feature at once, BCellobjects might received more data than required.
	 */
	@Override
	public boolean process() {
		...
		for (final BCellobjectAnalyzerFactory<?> factory : settings.getBCellobjectAnalyzerFactories()) {
			...
			model.getFeatureModel().declareBCellobjectFeatures(features, featureNames, featureShortNames,
	...
	public void computeBCellobjectFeatures(final BCellobjectCollection toCompute, final boolean doLogIt) {
		final List<BCellobjectAnalyzerFactory<?>> BCellobjectFeatureAnalyzers = settings
				.getBCellobjectAnalyzerFactories();
	 ...
	 * @param toCompute
	 */
	private void computeBCellobjectFeaturesAgent(final BCellobjectCollection toCompute,
			final List<BCellobjectAnalyzerFactory<?>> analyzerFactories, final boolean doLogIt) {
		...
		for (int ithread = 0; ithread < threads.length; ithread++) {
			...
			threads[ithread] = new Thread(
					"TrackMate BCellobject feature calculating thread " + (1 + ithread) + "/" + threads.length) {
				...
				public void run() {
					...
					for (int index = ai.getAndIncrement(); index < numFrames; index = ai.getAndIncrement()) {
						...
						for (final BCellobjectAnalyzerFactory<?> factory : analyzerFactories) {
							...
							final BCellobjectAnalyzer<?> analyzer = factory.getAnalyzer(model, img, frame,
									targetChannel);
