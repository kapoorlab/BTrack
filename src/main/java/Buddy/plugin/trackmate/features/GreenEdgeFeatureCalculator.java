 ...
 *
 */
public class GreenEdgeFeatureCalculator extends MultiThreadedBenchmarkAlgorithm {
	...
	public GreenEdgeFeatureCalculator(final GreenModel model, final GreenSettings settings) {
	 ...
	 * @param doLogIt
	 *            if <code>true</code>, the logger of the model will be notified of
	 *            the calculation.
	 */
	public void computeEdgesFeatures(final Collection<DefaultWeightedEdge> edges, final boolean doLogIt) {
		final List<GreenEdgeAnalyzer> spotFeatureAnalyzers = settings.getEdgeAnalyzers();
	...
	private void computeEdgeFeaturesAgent(final Collection<DefaultWeightedEdge> edges,
			final List<GreenEdgeAnalyzer> analyzers, final boolean doLogIt) {
		final Logger logger = model.getLogger();
		if (doLogIt) {
			logger.log("Computing edge features:\n", Logger.BLUE_COLOR);
		}
		for (final GreenEdgeAnalyzer analyzer : analyzers) {
