...
public class GreenEdgeTimeLocationAnalyzer implements GreenEdgeAnalyzer {
	...
	static {
		...
		FEATURE_DIMENSIONS.put(TIME, GREENDimension.TIME);
	...
	public GreenEdgeTimeLocationAnalyzer() {
		setNumThreads();
	}
			...
			threads[i] = new Thread("EdgeTimeLocationAnalyzer thread " + i) {
				...
				public void run() {
					...
					while ((edge = queue.poll()) != null) {
						...
						final double x = 0.5
								* (source.getFeature(Spot.POSITION_X) + target.getFeature(Spot.POSITION_X));
						final double y = 0.5
								* (source.getFeature(Spot.POSITION_Y) + target.getFeature(Spot.POSITION_Y));
						final double t = 0.5
								* (source.getFeature(Spot.POSITION_T) + target.getFeature(Spot.POSITION_T));
