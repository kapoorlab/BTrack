...
public class GreenEdgeVelocityAnalyzer implements GreenEdgeAnalyzer {
	...
	static {
		...
		FEATURE_DIMENSIONS.put(VELOCITY, GREENDimension.VELOCITY);
		FEATURE_DIMENSIONS.put(DISPLACEMENT, GREENDimension.LENGTH);
	...
	public GreenEdgeVelocityAnalyzer() {
		setNumThreads();
	}
			...
			threads[i] = new Thread("EdgeVelocityAnalyzer thread " + i) {
				...
				public void run() {
					...
					while ((edge = queue.poll()) != null) {
						...
						final double dx = target.diffTo(source, Greenobject.POSITION_X);
						final double dy = target.diffTo(source, Greenobject.POSITION_Y);
						final double dz = target.diffTo(source, Greenobject.POSITION_Z);
						final double dt = target.diffTo(source, Greenobject.POSITION_T);
