...
public class GreenTrackMateWizard extends JFrame implements ActionListener {
	...
	public static final ImageIcon TRACKMATE_ICON = new ImageIcon(
			TrackMateWizard.class.getResource("images/Logo50x50-color-nofont-72p.png"));
	...
	private static final Icon DISPLAY_CONFIG_ICON = new ImageIcon(
			TrackMateWizard.class.getResource("images/wrench_orange.png"));
	...
	/**
	 * Sets the current panel to that identified by the WizardPanelDescriptor passed
	 * in.
	 * 
	 * @param descriptor
	...
	private void initGUI() {
		try {
			...
			setTitle(Buddy.plugin.trackmate.TrackMate.PLUGIN_NAME_STR + " v"
					+ Buddy.plugin.trackmate.TrackMate.PLUGIN_NAME_VERSION);
			{
			...
			/*
			 * We don't show the load button anymore. Load action can be accessed from the
			 * menu. Out of simplicity, we leave the load button logic in place, but do not
			 * show it.
			 */
	...
	private JButton addButton(final String label, final Icon icon, final int x, final int y, final int width,
			final int height, final ActionEvent action) {
