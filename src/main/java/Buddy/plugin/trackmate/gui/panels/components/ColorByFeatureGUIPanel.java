...
public class ColorByFeatureGUIPanel extends ActionListenablePanel implements MinMaxAdjustable {
	...
	private void repaintColorCanvas(final Graphics g) {
		if (null == jComboBoxSetColorBy.getSelectedItem() || getColorGeneratorCategory().equals(Category.DEFAULT)
				|| (getColorGeneratorCategory().equals(Category.BCellobject)
						&& getColorFeature().equals(ManualBCellobjectColorAnalyzerFactory.FEATURE))
	...
	protected CategoryJComboBox<Category, String> createComboBoxSelector(final List<Category> lCategories) {
		...
		for (final Category category : lCategories) {
			...
			case BCellobject:
				...
				final Collection<String> BCellobjectFeatures = new ArrayList<>(
						model.getFeatureModel().getBCellobjectFeatures());
