...
public class GreenFilterGuiPanel extends ActionListenablePanel implements ChangeListener {
	...
	private GreenColorByFeatureGUIPanel jPanelColorByFeatureGUI;
	...
	public GreenFilterGuiPanel(final GreenModel model, final List<Category> categories) {
		this.model = model;
		...
		this.updater = new OnRequestUpdater(new OnRequestUpdater.Refreshable() {
			@Override
			public void refresh() {
				GreenFilterGuiPanel.this.refresh();
			}
		});
	...
	private void initGUI() {
		try {
			...
			}
			{
				...
				{
					jPanelColorByFeatureGUI = new GreenColorByFeatureGUIPanel(model, categories);
