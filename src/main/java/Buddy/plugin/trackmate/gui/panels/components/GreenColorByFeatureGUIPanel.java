...
import Buddy.plugin.trackmate.visualization.MinMaxAdjustable;

public class GreenColorByFeatureGUIPanel extends ActionListenablePanel implements MinMaxAdjustable {
	...
	public static enum Category {
		Greenobject("Greenobject"), EDGES("edges"), TRACKS("tracks"), DEFAULT("Default");
	...
	public GreenColorByFeatureGUIPanel(final GreenModel model, final List<Category> categories) {
		super();
		this.model = model;
	...
	/**
	 * Returns a key to the color generator category selected in the combo box. Will
	 * be a {@link Category} enum type, as set in constructor.
	 *
	...
	private void repaintColorCanvas(final Graphics g) {
		if (null == jComboBoxSetColorBy.getSelectedItem() || getColorGeneratorCategory().equals(Category.DEFAULT)
				|| (getColorGeneratorCategory().equals(Category.Greenobject)
						&& getColorFeature().equals(ManualGreenobjectColorAnalyzerFactory.FEATURE))
				|| (getColorGeneratorCategory().equals(Category.EDGES)
						&& getColorFeature().equals(ManualEdgeColorAnalyzer.FEATURE))) {
		...
		case EDGES:
			...
			break;
		case Greenobject:
			isInt = model.getFeatureModel().getGreenobjectFeatureIsInt().get(feature);
			break;
		default:
			isInt = false;
		}
	 ...
	 * Subclasses can override this method to decide what items are in the combo box
	 * list.
	 *
	...
	protected CategoryJComboBox<Category, String> createComboBoxSelector(final List<Category> lCategories) {
		...
		for (final Category category : lCategories) {
			switch (category) {
			case Greenobject:
				categoryNames.put(Category.Greenobject, "Greenobject features:");
				final Collection<String> GreenobjectFeatures = new ArrayList<>(
						model.getFeatureModel().getGreenobjectFeatures());
				features.put(Category.Greenobject, GreenobjectFeatures);
				// Deal with manual coloring separately.
				GreenobjectFeatures.remove(ManualGreenobjectColorAnalyzerFactory.FEATURE);

				featureNames.putAll(model.getFeatureModel().getGreenobjectFeatureNames());
				break;
	...
	protected double[] getValues(final CategoryJComboBox<Category, String> cb) {
		...
		case EDGES:
			...
			break;
		case Greenobject:
			final GreenobjectCollection Greenobject = model.getGreenobjects();
			values = Greenobject.collectValues(feature, false);
			break;
		case DEFAULT:
