...
public class ExtractTrackStackAction extends AbstractTMAction {
	...
	public static final String INFO_TEXT = "<html> " + "Generate a stack of images taken from the track "
			+ "that joins two selected BCellobjects. " + "<p> "
			...
			+ "All channels are captured. " + "</html>";
	...
	public void execute(final TrackMate trackmate) {
		...
		if (nBCellobjects != 2) {
			if (nBCellobjects == 1) {
				final Integer trackID = model.getTrackModel()
						.trackIDOf(selectionModel.getBCellobjectSelection().iterator().next());
				final List<BCellobject> BCellobjects = new ArrayList<>(
						model.getTrackModel().trackBCellobjects(trackID));
		...
		for (final DefaultWeightedEdge edge : edges) {
			...
			if (current == previous) {
				current = model.getTrackModel().getEdgeTarget(edge); // We have to check both in case of bad oriented
																		// edges
			}
			path.add(current);
		...
		for (final BCellobject BCellobject : sortedBCellobjects) {
			...
			for (int c = 0; c < nChannels; c++) {
				...
				final int x = (int) (Math.round((BCellobject.getFeature(BCellobject.POSITION_X)) / calibration[0])
						- width / 2);
				final int y = (int) (Math.round((BCellobject.getFeature(BCellobject.POSITION_Y)) / calibration[1])
						- height / 2);
				long slice = 0;
