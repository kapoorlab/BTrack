...
public class LabelImgExporter extends AbstractTMAction {
	...
	public static final String INFO_TEXT = "<html>" + "This action creates a label image from the tracking results. "
			...
			+ "larger than the last trackID in the dataset. " + "<p> " + "Only visible BCellobjects are painted. "
			+ "</html>";
	...
	public void execute(final TrackMate trackmate) {
		...
		if (gui != null) {
			...
			final int userInput = JOptionPane.showConfirmDialog(gui, panel, "Export to label image",
					JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, TrackMateWizard.TRACKMATE_ICON);
	 ...
	 * model are painted as ellipsoids taken from their shape, with their track ID
	 * as pixel value.
	 *
	 * @param trackmate
	 *            the trackmate instance from which we takes the BCellobjects to
	 *            paint. The label image will have the same calibration, name and
	 *            dimension from the input image stored in the trackmate settings.
	 *            The output label image will have the same size that of this input
	 *            image, except for the number of channels, which will be 1.
	 ...
	 *            belonging to a track will be painted with a unique ID, different
	 *            from the track IDs and different for each BCellobject.
	 *
	 * @return a new {@link ImagePlus}.
	 */
	public static final ImagePlus createLabelImagePlus(final TrackMate trackmate,
			final boolean exportBCellobjectsAsDots, final boolean exportTracksOnly) {
	 ...
	 */
	public static final ImagePlus createLabelImagePlus(final TrackMate trackmate,
			final boolean exportBCellobjectsAsDots, final boolean exportTracksOnly, final Logger logger) {
		return createLabelImagePlus(trackmate.getModel(), trackmate.getSettings().imp, exportBCellobjectsAsDots,
	 ...
	 * @param imp
	 *            a source image to read calibration, name and dimension from. The
	 *            output label image will have the same size that of this source
	 *            image, except for the number of channels, which will be 1.
	 * @param exportBCellobjectsAsDots
	 *            if <code>true</code>, BCellobjects will be painted as single dots
	 *            instead of ellipsoids.
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final ImagePlus imp,
			final boolean exportBCellobjectsAsDots, final boolean exportTracksOnly) {
		return createLabelImagePlus(model, imp, exportBCellobjectsAsDots, exportTracksOnly, Logger.VOID_LOGGER);
	}
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final ImagePlus imp,
		...
		final ImagePlus lblImp = createLabelImagePlus(model, dims, exportBCellobjectsAsDots, exportTracksOnly, logger);
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final int[] dimensions,
			final boolean exportBCellobjectsAsDots, final boolean exportTracksOnly) {
		return createLabelImagePlus(model, dimensions, exportBCellobjectsAsDots, exportTracksOnly, Logger.VOID_LOGGER);
	}
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final int[] dimensions,
		...
		final ImagePlus lblImp = ImageJFunctions
				.wrap(createLabelImg(model, dims, exportBCellobjectsAsDots, exportTracksOnly, logger), "LblImage");
	 ...
	 * BCellobjects of the specified model are painted as ellipsoids taken from
	 * their shape, with their track ID as pixel value.
	 *
	 * @param model
	 *            the model from which we takes the BCellobjects to paint.
	 * @param dimensions
	...
	public static final Img<UnsignedShortType> createLabelImg(final Model model, final long[] dimensions,
			final boolean exportBCellobjectsAsDots, final boolean exportTracksOnly) {
		return createLabelImg(model, dimensions, exportBCellobjectsAsDots, exportTracksOnly, Logger.VOID_LOGGER);
	}
	...
	public static final Img<UnsignedShortType> createLabelImg(final Model model, final long[] dimensions,
		...
		for (int frame = 0; frame < dimensions[3]; frame++) {
			...
			final BCellobjectWriter BCellobjectWriter = exportBCellobjectsAsDots ? new BCellobjectAsDotWriter(imgCT)
	...
	private static final class BCellobjectSphereWriter implements BCellobjectWriter {
		...
		public void write(final BCellobject BCellobject, final int id) {
			final BCellobjectNeighborhood<UnsignedShortType> neighborhood = new BCellobjectNeighborhood<UnsignedShortType>(
					BCellobject, img);
	...
	private static final class BCellobjectAsDotWriter implements BCellobjectWriter {
		...
		public BCellobjectAsDotWriter(final ImgPlus<UnsignedShortType> img) {
	...
	public static void main(final String[] args) {
		...
		createLabelImagePlus(plugIn.getModel(), plugIn.getSettings().imp, false, true, plugIn.getModel().getLogger())
				.show();
	}
}
