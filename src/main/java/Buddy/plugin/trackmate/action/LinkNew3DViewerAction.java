...
public class LinkNew3DViewerAction extends AbstractTMAction {
	...
	private static final String INFO_TEXT = "<html>"
			...
			+ "properly linked to the current controller." + "<p>"
	...
	public void execute(final TrackMate trackmate) {
		new Thread("TrackMate new 3D viewer thread") {
			...
			public void run() {
				...
				win.pack();
				win.setVisible(true);
				final BCellobjectDisplayer3D newDisplayer = new BCellobjectDisplayer3D(trackmate.getModel(),
						controller.getSelectionModel(), universe);
