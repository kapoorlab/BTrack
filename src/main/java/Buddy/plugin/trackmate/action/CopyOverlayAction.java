...
import Buddy.plugin.trackmate.visualization.threedviewer.BCellobjectDisplayer3DFactory;
...
public class CopyOverlayAction extends AbstractTMAction {
	...
	public static final String INFO_TEXT = "<html>"
			...
			+ "displayed on a modified image. " + "<p>"
			+ "The new view will be independent, and will have its own control panel.<br> " + "</html>";
				...
				if (e == impChooser.OK_BUTTON_PUSHED) {
					new Thread("TrackMate copying thread") {
						...
						public void run() {
							...
							final BCellobjectColorGenerator BCellobjectColorGenerator = new BCellobjectColorGenerator(
									tm.getModel());
							final PerTrackFeatureColorGenerator trackColorGenerator = new PerTrackFeatureColorGenerator(
									tm.getModel(), TrackIndexAnalyzer.TRACK_INDEX);
							final PerEdgeFeatureColorGenerator edgeColorGenerator = new PerEdgeFeatureColorGenerator(
									tm.getModel(), EdgeVelocityAnalyzer.VELOCITY);
							final ManualEdgeColorGenerator manualEdgeColorGenerator = new ManualEdgeColorGenerator(
									tm.getModel());
							final ManualBCellobjectColorGenerator manualBCellobjectColorGenerator = new ManualBCellobjectColorGenerator();
							final BCellobjectColorGeneratorPerTrackFeature BCellobjectColorGeneratorPerTrackFeature = new BCellobjectColorGeneratorPerTrackFeature(
									tm.getModel(), TrackIndexAnalyzer.TRACK_INDEX);
