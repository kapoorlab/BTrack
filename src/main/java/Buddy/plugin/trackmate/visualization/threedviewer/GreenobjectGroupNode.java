...
public class GreenobjectGroupNode<K> extends ContentNode {
	...
	/**
	 * Hold the center and radius position of all Greenobjects.
	 */
	protected Map<K, Point4d> centers;

	/**
	 * Hold the color and transparency of all Greenobjects.
	 */
	protected Map<K, Color4f> colors;

	/**
	 * Hold the mesh of each Greenobject.
	 */
	protected HashMap<K, CustomTriangleMesh> meshes;

	/**
	 * Hold the text of each Greenobject.
	 */
	protected Map<K, TransformGroup> texts;

	/**
	 * Switch used for Greenobject display.
	 */
	protected Switch GreenobjectSwitch;

	/**
	 * Switch used for Greenobject names display.
	 */
	protected Switch textSwitch;

	/**
	 * Boolean set that controls the visibility of each Greenobject.
	 */
	protected BitSet switchMask;

	/**
	 * Map that links the Greenobject keys to their indices in the Switch.
	 *
	 * @see #GreenobjectSwitch
	 */
	...
	/**
	 * Create a new {@link GreenobjectGroupNode} with Greenobjects at position and
	 * with color given in argument.
	 * <p>
	 * The positions are given by a {@link Point4d} map. The <code>x</code>,
	 * <code>y</code>, <code>z</code> are used to specify the Greenobject center,
	 ...
	 * @param colors
	 */
	public GreenobjectGroupNode(final Map<K, Point4d> centers, final Map<K, Color4f> colors) {
		this.centers = new HashMap<>(centers);
		this.colors = new HashMap<>(colors);
		//
		this.GreenobjectSwitch = new Switch(Switch.CHILD_MASK);
		GreenobjectSwitch.setCapability(Switch.ALLOW_SWITCH_WRITE);
		GreenobjectSwitch.setCapability(Group.ALLOW_CHILDREN_WRITE);
		GreenobjectSwitch.setCapability(Group.ALLOW_CHILDREN_EXTEND);
		//
	 ...
	 * Greenobjects, with a transparency of 0.
	 * <p>
	 * The arguments are copied on creation, ensuring that are unmodified by this
	 * class, and vice-versa.
	 *
	 * @param centers
	 *            a map that maps Greenobject to their centers as Point4d.
	 * @param color
	 *            the Greenobject color as Color3f.
	 */
	public GreenobjectGroupNode(final HashMap<K, Point4d> centers, final Color3f color) {
		this.centers = new HashMap<>(centers);
	...
	public String toString() {
		...
		str.append("GreenobjectGroupNode with " + centers.size() + " Greenobjects.\n");
		...
		//
		str.append("  - content:\n");
		for (final K Greenobject : centers.keySet()) {
			final int index = indices.get(Greenobject);
			str.append("     - " + Greenobject + ": color = " + colors.get(Greenobject) + "; center = "
					+ centers.get(Greenobject) + "; visible = " + switchMask.get(index) + "\n");
		}
		return str.toString();
	}
	...
	/**
	 * Re-create the {@link #meshes} field with Greenobject meshes, from the fields
	 * {@link #centers} and {@link #colors}.
	 * <p>
	 * This resets the {@link #GreenobjectSwitch} and the {@link #switchMask} fields
	 * with new values.
	 */
	protected void makeMeshes() {
		...
		GreenobjectSwitch.removeAllChildren();
		...
		for (final K key : centers.keySet()) {
			...
			bg.addChild(node);
			GreenobjectSwitch.addChild(bg); // at index
		...
		GreenobjectSwitch.setChildMask(switchMask);
		if (showLabels) {
	...
	public void add(final K key, final Point4d center, final Color4f color) {
		...
		bg1.addChild(node);
		GreenobjectSwitch.addChild(bg1);
	...
	public void remove(final K key) {
		...
		// Remove Greenobject from scene
		final CustomTriangleMesh mesh = meshes.remove(key);
		GreenobjectSwitch.removeChild(mesh.getParent());
		...
		GreenobjectSwitch.setChildMask(bitSet);
		if (showLabels) {
	...
	/**
	 * Set the visibility of all Greenobjects given in argument to
	...
	/**
	 * Set the visibility of all Greenobjects.
	 */
	...
	/**
	 * Set the visibility of the Greenobject <code>key</code>.
	 */
	...
	/**
	 * Set the color of all Greenobjects.
	 */
	...
	/**
	 * Set the color of the Greenobject <code>key</code>. Its transparency is
	 * unchanged.
	 */
	...
	/**
	 * Set the color of the Greenobject <code>key</code>. Its transparency set by
	...
	/**
	 * Set the transparency of the Greenobject <code>key</code>. Its color is
	 * unchanged.
	 */
	...
	/**
	 * Move the Greenobject <code>key</code> center to the position given by the
	...
	/**
	 * Change the radius of the Greenobject <code>key</code>. Its position is
	 * unchanged.
	 */
