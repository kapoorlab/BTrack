 ...
 */
public class GreenTrackOverlay extends Roi {
	...
	public GreenTrackOverlay(final GreenModel model, final ImagePlus imp, final Map<String, Object> displaySettings) {
		super(0, 0, imp);
		this.model = model;
	...
	public final synchronized void drawOverlay(final Graphics g) {
		...
		case TrackMateModelView.TRACK_DISPLAY_MODE_SELECTION_ONLY: {
			for (final DefaultWeightedEdge edge : highlight) {
				...
				source = model.getTrackModel().getEdgeSource(edge);
				final int sourceFrame = source.getFeature(Greenobject.POSITION_T).intValue();
	...
	private static final boolean isOnClip(final Greenobject source, final Greenobject target, final double minx,
			final double miny, final double maxx, final double maxy, final double[] calibration) {
		// Find x & y in physical coordinates
		final double x0i = source.getFeature(Greenobject.POSITION_X);
		final double y0i = source.getFeature(Greenobject.POSITION_Y);
		final double x1i = target.getFeature(Greenobject.POSITION_X);
		final double y1i = target.getFeature(Greenobject.POSITION_Y);
		// In pixel units
		...
		// Is any Greenobject inside the clip?
	...
	protected void drawEdge(final Graphics2D g2d, final Greenobject source, final Greenobject target, final int xcorner,
