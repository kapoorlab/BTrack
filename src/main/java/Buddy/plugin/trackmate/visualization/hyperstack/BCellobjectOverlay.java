...
public class BCellobjectOverlay extends Roi {
	...
	public void drawOverlay(final Graphics g) {
		...
		final FeatureColorGenerator<BCellobject> colorGenerator = (FeatureColorGenerator<BCellobject>) displaySettings
		...
		if (selectionOnly && null != BCellobjectSelection) {
			...
			for (final Iterator<BCellobject> iterator = BCellobjects.iterator(frame); iterator.hasNext();) {
				...
				if (editingBCellobject == BCellobject
						|| (BCellobjectSelection != null && BCellobjectSelection.contains(BCellobject))) {
					continue;
				}
			...
			if (null != BCellobjectSelection) {
				...
				for (final BCellobject BCellobject : BCellobjectSelection) {
					...
					final int sFrame = BCellobject.getFeature(BCellobject.POSITION_T).intValue();
					if (DEBUG) {
						System.out.println("[BCellobjectOverlay] For BCellobject " + BCellobject
	...
	protected void drawBCellobject(final Graphics2D g2d, final BCellobject BCellobject, final double zslice,
		...
		final boolean BCellobjectNameVisible = (Boolean) displaySettings
				.get(TrackMateModelView.KEY_DISPLAY_BCellobject_NAMES);
