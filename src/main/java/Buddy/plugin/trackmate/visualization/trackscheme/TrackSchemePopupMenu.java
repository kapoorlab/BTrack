	...
	private void init() {
		...
		if (cell != null) {
			...
			final Action linkAction = new AbstractAction(
					"Link " + trackScheme.getSelectionModel().getBCellobjectSelection().size() + " BCellobjects") {
				@Override
				public void actionPerformed(final ActionEvent e) {
					linkBCellobjects();
				}
		...
		if (vertices.size() > 0) {
			final String str = "Manual color for "
					+ (vertices.size() == 1 ? " one BCellobject" : vertices.size() + " BCellobjects");
		...
		if (edges.size() > 0 && vertices.size() > 0) {
			final String str = "Manual color for "
					+ (vertices.size() == 1 ? " one BCellobject and " : vertices.size() + " BCellobjects and ")
