...
public class GreenTrackSchemePopupMenu extends JPopupMenu {
	...
	public GreenTrackSchemePopupMenu(final GreenTrackScheme trackScheme, final Object cell, final Point point) {
	...
	private void manualColorVertices(final ArrayList<mxCell> vertices) {
		for (final mxCell mxCell : vertices) {
			final Greenobject BCellobject = trackScheme.getGraph().getGreenobjectFor(mxCell);
			final Double value = Double.valueOf(previousColor.getRGB());
		...
		graphComponent.addListener(mxEvent.LABEL_CHANGED, new mxIEventListener() {
			...
			public void invoke(final Object sender, final mxEventObject evt) {
				for (final mxCell lCell : vertices) {
					...
					trackScheme.getGraph().getGreenobjectFor(lCell).setName(tc.getValue().toString());
				}
	...
	private void init() {
		...
		if (cell != null) {
			// Edit
			add(new AbstractAction("Edit Greenobject name") {
				@Override
				public void actionPerformed(final ActionEvent e) {
					editBCellobjectName();
				}
			...
			if (vertices.size() > 1) {
				...
				// Multi edit
				add(new AbstractAction("Edit " + vertices.size() + " Greenobject names") {
					@Override
					public void actionPerformed(final ActionEvent e) {
			...
			final Action linkAction = new AbstractAction(
					"Link " + trackScheme.getSelectionModel().getGreenobjectSelection().size() + " Greenobjects") {
				@Override
				public void actionPerformed(final ActionEvent e) {
					linkBCellobjects();
				}
			};
			if (trackScheme.getSelectionModel().getGreenobjectSelection().size() > 1) {
				add(linkAction);
			}
		}
		...
		if (vertices.size() > 0) {
			final String str = "Manual color for "
					+ (vertices.size() == 1 ? " one Greenobject" : vertices.size() + " Greenobjects");
		...
		if (edges.size() > 0 && vertices.size() > 0) {
			final String str = "Manual color for "
					+ (vertices.size() == 1 ? " one Greenobject and " : vertices.size() + " Greenobjects and ")
		...
		if (selection.length > 0) {
			addSeparator();
			final Action removeAction = new AbstractAction("Remove Greenobjects and links") {
				@Override
				public void actionPerformed(final ActionEvent e) {
					remove();
				}
