 ...
 *
 */
public class GreenTrackSchemeGraphLayout extends mxGraphLayout implements Benchmark {

	private static final int START_COLUMN = 1;

	/** The target model to draw Greenobject from. */
	...
	public void execute(final Object lParent) {
		 ...
		 */
		final GreenTimeDirectedNeighborIndex neighborCache = model.getTrackModel().getDirectedNeighborIndex();

		/*
		 * Compute column width from recursive cumsum
		 */
		final Map<Greenobject, Integer> cumulativeBranchWidth = GreenGraphUtils.cumulativeBranchWidth(model.getTrackModel());

		/*
		 * How many rows do we have to parse?
		 */
		final int maxFrame = model.getGreenobjects().lastKey();
		...
		try {
			...
			for (final Integer trackID : model.getTrackModel().trackIDs(true)) { // will be sorted by track name
				...
				// Get first Greenobject
				final TreeSet<Greenobject> sortedTrack = new TreeSet<>(Greenobject.frameComparator);
				sortedTrack.addAll(track);
				final Greenobject first = sortedTrack.first();

				/*
				 * A special case: our quick layout below fails for graph that are not trees.
				 * That is: if a track has at least a Greenobject that has more than one
				...
				if (isTree) {
					...
					while (iterator.hasNext()) {
						...
						final mxICell cell = graphAdapter.getCellFor(Greenobject);

						// This is cell is in a track, remove it from the list
						// of lonely cells
						lonelyCells.remove(cell);

						// Determine in what row to put the Greenobject
						final int frame = Greenobject.getFeature(Greenobject.POSITION_T).intValue();

						// Cell size, position and style
						final int cellPos = columns[frame] + cumulativeBranchWidth.get(Greenobject) / 2;
						setCellGeometry(cell, frame, cellPos);
						columns[frame] += cumulativeBranchWidth.get(Greenobject);

						// If it is a leaf, we fill the remaining row below and
						// above
						if (neighborCache.successorsOf(Greenobject).size() == 0) {
					...
					while (depthFirstIterator.hasNext()) {
						final List<Greenobject> branch = depthFirstIterator.next();

						final int firstFrame = branch.get(0).getFeature(Greenobject.POSITION_T).intValue();
						final int lastFrame = branch.get(branch.size() - 1).getFeature(Greenobject.POSITION_T)
								.intValue();

						// Determine target column.
						int targetColumn = columns[firstFrame];
						for (final Greenobject Greenobject : branch) {
			...
			for (final mxCell cell : lonelyCells) {
				final Greenobject Greenobject = graphAdapter.getGreenobjectFor(cell);
				final int frame = Greenobject.getFeature(Greenobject.POSITION_T).intValue();
