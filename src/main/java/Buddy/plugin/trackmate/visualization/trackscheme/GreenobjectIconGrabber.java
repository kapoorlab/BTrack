...
/**
 * This class is used to take a snapshot of a {@link Greenobject} object (or
 * collection) from its coordinates and an {@link ImagePlus} that contain the
 * pixel data.
 *
 * @author Jean-Yves Tinevez - Dec 2010 - 2017
 */
public class GreenobjectIconGrabber<T extends RealType<T>> {

	private final ImgPlus<T> img;

	public GreenobjectIconGrabber(final ImgPlus<T> img) {
		this.img = img;
	}

	/**
	 * Returns the image string for the specified Greenobject. The Greenobject x,y,z
	 * and radius coordinates are used to get a location on the image given at
	 ...
	 *
	 * @param Greenobject
	 *            the Greenobject to generate a thumbnail image from.
	 * @param radiusFactor
	...
	public String getImageString(final Greenobject Greenobject, final double radiusFactor) {
		...
		final double radius = Greenobject.getFeature(Greenobject.RADIUS) * radiusFactor;
		final long x = Math.round((Greenobject.getFeature(Greenobject.POSITION_X) - radius) / calibration[0]);
		final long y = Math.round((Greenobject.getFeature(Greenobject.POSITION_Y) - radius) / calibration[1]);
