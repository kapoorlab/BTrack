...
public class GreenInfoPane extends JPanel implements GreenSelectionChangeListener {
	...
	/**
	 * A copy of the last Greenobject collection highlighted in this infopane,
	 * sorted by frame order.
	 */
	private Collection<Greenobject> GreenobjectSelection;

	private final OnRequestUpdater updater;

	/** The table headers, taken from Greenobject feature names. */
	...
	/**
	 * Creates a new Info pane that displays information on the current Greenobject
	 * selection in a table.
	 *
	 * @param model
	 *            the {@link Model} from which the Greenobject collection is taken.
	 * @param selectionModel
	 *            the {@link SelectionModel} from which we read what to show in the
	 *            table.
	 */
	public GreenInfoPane(final GreenModel model, final GreenSelectionModel selectionModel) {
		this.model = model;
		this.selectionModel = selectionModel;
		final List<String> features = new ArrayList<>(model.getFeatureModel().getGreenobjectFeatures());
		final Map<String, String> featureNames = model.getFeatureModel().getGreenobjectFeatureShortNames();
		...
		addAncestorListener(new AncestorListener() {
			...
			public void ancestorRemoved(final AncestorEvent event) {
				GreenInfoPane.this.selectionModel.removeSelectionChangeListener(GreenInfoPane.this);
			}
	...
	public void selectionChanged(final GreenSelectionChangeEvent event) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				highlightGreenobjects(selectionModel.getGreenobjectSelection());
			}
		});
	}

	/**
	 * Show the given Greenobject selection as a table displaying their individual
	 * features.
	 */
	private void highlightGreenobjects(final Collection<Greenobject> Greenobjects) {
		if (!doHighlightSelection)
			return;
		GreenobjectSelection = Greenobjects;
	...
	private void update() {
		 ...
		 * likely to happen.
		 */
		final List<Greenobject> sortedGreenobjects = new ArrayList<>(GreenobjectSelection);
		Collections.sort(sortedGreenobjects, Greenobject.frameComparator);
		...
		final List<String> features = new ArrayList<>(model.getFeatureModel().getGreenobjectFeatures());
		for (final Greenobject Greenobject : sortedGreenobjects) {
			if (null == Greenobject) {
				continue;
			}
			final Object[] columnData = new Object[features.size() + 1];
			columnData[0] = String.format("%d", model.getTrackModel().trackIDOf(Greenobject));
			for (int i = 1; i < columnData.length; i++) {
				final String feature = features.get(i - 1);
				final Double feat = Greenobject.getFeature(feature);
				if (null == feat) {
					columnData[i] = "";
				} else if (model.getFeatureModel().getGreenobjectFeatureIsInt().get(feature).booleanValue()) {
					columnData[i] = "" + feat.intValue();
				} else {
					columnData[i] = String.format("%.4g", feat.doubleValue());
				}
			}
			dm.addColumn(Greenobject.toString(), columnData);
		}
		table.setModel(dm);
	...
	private void exportTableToImageJ() {
		...
		final List<String> features = new ArrayList<>(fm.getGreenobjectFeatures());

		final int ncols = GreenobjectSelection.size();
		final int nrows = headers.length;
		final Greenobject[] GreenobjectArray = GreenobjectSelection.toArray(new Greenobject[] {});
		...
		for (int i = 0; i < ncols; i++) {
			final Greenobject Greenobject = GreenobjectArray[i];
			final Integer trackID = model.getTrackModel().trackIDOf(Greenobject);
			if (null == trackID) {
				lTable.addValue(Greenobject.getName(), "None");
			} else {
				lTable.addValue(Greenobject.getName(), "" + trackID.intValue());
			}
		}
		...
		for (int j = 0; j < nrows - 1; j++) {
			...
			for (int i = 0; i < ncols; i++) {
				...
				if (val == null) {
					...
					if (fm.getGreenobjectFeatureIsInt().get(feature)) {
						lTable.addValue(Greenobject.getName(), "" + val.intValue());
					} else {
						lTable.addValue(Greenobject.getName(), val.doubleValue());
					}
				}
			}
		}
	...
	private void init() {
		...
		final List<String> features = new ArrayList<>(model.getFeatureModel().getGreenobjectFeatures());
		final Map<String, String> featureNames = model.getFeatureModel().getGreenobjectFeatureShortNames();
		featureSelectionPanel = new FeaturePlotSelectionPanel(Greenobject.POSITION_T, features, featureNames);
	...
	/**
	 * Reads the content of the current Greenobject selection and plot the selected
	 * features in this {@link InfoPane} for the target Greenobjects.
	 *
	 * @param xFeature
	...
	private void plotSelectionData(final String xFeature, final Set<String> yFeatures) {
		final Set<Greenobject> Greenobjects = selectionModel.getGreenobjectSelection();
		if (yFeatures.isEmpty() || Greenobjects.isEmpty()) {
			return;
		}

		final GreenobjectFeatureGrapher grapher = new GreenobjectFeatureGrapher(xFeature, yFeatures, Greenobjects,
				model);
		grapher.render();
	}
